...
def obj_to_cont(
    ...
    Args:
        vars (List[str]): input variables
        df (pd.DataFrame): input dataframe
        mapping (Dict[str, int]): mapping dictionary
...
def feature_engineering_hh(df: pd.DataFrame) -> pd.DataFrame:
    """Derive features from the household level data

    Args:
        df (pd.DataFrame): input dataframe

    Returns:
        pd.DataFrame: processed dataframe with derived features
    """
    ...
    heads = heads[
        config.processing.CLIENT_UNIQUE_IDENTIFER
        + [config.processing.TARGET_VAR]
        + config.processing.HH_BOOL_VARS
        + config.processing.HH_CONT_VARS
        + config.processing.HH_ORDINAL_VARS
    ]

    # Drop redundant household-level variables
    heads = heads.drop(columns=config.processing.REDUNDANT_HH_VARS)
...
def feature_engineering_ind(df: pd.DataFrame) -> pd.DataFrame:
    """Pefforms feature engineering and selection for individual-level variables
    ...
    ind = df[
        config.processing.CLIENT_UNIQUE_IDENTIFER
        + [config.processing.TARGET_VAR]
        + config.processing.IND_BOOL_VARS
        + config.processing.IND_ORDINAL_VARS
    ].copy()
...
def data_preprocessing(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.drop(columns=config.processing.SQUARED_VARS)
